...
import { WORLD_WIDTH, WORLD_HEIGHT } from './WorldBackdrop.js';
...
const GENERATE_MOUNTAINS = false;
const GENERATE_ROADS = false;
...
function buildRoadEntities(villageRects, fieldRects, mountainObstacles) {
  ...
  for (let i = 0; i < rects.length; i++) {
    ...
    for (let j = i + 1; j < rects.length; j++) {
      ...
      if (hops[j] <= EXTRA_ROAD_MIN_HOPS) continue;
